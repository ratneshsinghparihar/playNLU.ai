...
import services.rsNLPStanfordWraper._


import scala.reflect.runtime.universe.TypeTag

class rsNLPStanforWrapperTest extends BaseSparkSetupForTest {
  ...
  private def testMultiFunction[T: TypeTag](function1: UserDefinedFunction,
                                            function2: UserDefinedFunction,
                                            function3: UserDefinedFunction,
                                            function4: UserDefinedFunction,
                                            input: T, expected: Any): Unit = {
    val df = sqlContext.createDataFrame(Seq((0, input))).toDF("id", "input")
    val curObj=df.select(function1(col("input")),function2(col("input")),function3(col("input")),function3(col("input")))
    return  curObj;
  }

  test("multi") {
    testMultiFunction(tokenize,sentiment,ner,pos,"Dalia Asterbadi is very bad at studying but awesome in sports. he is visiting Toronto","")
  }
  ...
  test("openie") {
    ...
    /*var caseted=expected.map { case Row(subject: String, relation: String, target: String, confidence: Double)
    => OpenIE(subject,relation,target,confidence) }*/
  }
